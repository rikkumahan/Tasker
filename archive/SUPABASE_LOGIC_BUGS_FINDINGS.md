...
**Status:** ✅ Fixed — `supabase/functions/_shared/auth.ts` now independently verifies HS256 signatures via native Web Crypto (`crypto.subtle.verify`) against `SUPABASE_JWT_SECRET`, rejects `alg` values other than `HS256` (alg-confusion), and checks `exp`. `synthesize_profile/index.ts`'s duplicate unverified decode block deleted, now calls `authenticateUser()`. Regression tests: `supabase/functions/tests/test-auth.ts` (forged-signature and alg:none rejection — pure unit tests, no DB needed). **Requires `SUPABASE_JWT_SECRET` to be set as a function secret before deploy** — without it, `authenticateUser` logs an error and falls back to `auth.getUser()` only (fails closed, not open).
